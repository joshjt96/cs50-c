...
bool vote(int voter, int rank, string name)
{
    ...
    {
        // Check candidate array for matching name
        if (strcmp(name, candidates[c].name) == 0)
        {
            // If match, add chosen candidate to preference array
...
bool print_winner(void)
{
    for (int c = 0; c < candidate_count; c++)
    {
        // for each candidate in the candidate_count array;
        // if a candidate's number of votes is greater than the voter count divided by 2 then they have the most votes.
